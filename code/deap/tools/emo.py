...
def sortPOprob(individuals, k, first_front_only=False):
    ...
    fit_array = numpy.array([ind.fitness.values for ind in individuals])
    ...
    num_unique = len(set([str(ind) for ind in individuals]))
    if num_unique != pop_size:
        v = 1
        pass

    ind_to_po_val = list(zip(individuals, po_vals))
    ...
    for i in range(m + 1, pop_size):
        ...
        if po_val == curr_po_level:
            ...
            break
        pass
    # print('Size of the first front {}'.format(len(fronts[0])))
    # calc avg num of individuals per front apart from the 1st front
    tot = 0
    for i in range(1, len(fronts)):
        tot += len(fronts[i])
        pass
    # print('\tnum fronts {}'.format(len(fronts)))
    # print('\tnum ind per front {}'.format(tot * 1. / (len(fronts) - 1)))
    return fronts


def sortPO(individuals, k, first_front_only=False):
    """Sort the first *k* *individuals* into different nondomination levels.
    ...
    dominating_list = list(dominating_fits.items())
    dominating_list.sort(key=lambda x: x[1])

    fronts = []
    curr_po_level = -1
    N = min(len(individuals), k)
    pareto_sorted = 0
    for m in range(0, len(dominating_list)):
        el = dominating_list[m]
        fit = el[0]
        po_val = el[1]
        if first_front_only and po_val == 1:
            break
        if curr_po_level < po_val:
            # create new front
            curr_po_level = po_val
        ...
        pareto_sorted += len(map_fit_ind[fit])
        if pareto_sorted >= N:
            break
        pass
    # add all elements with the same value of po
    for i in range(m + 1, len(dominating_list)):
        el = dominating_list[i]
        po_val = el[1]
        if po_val == curr_po_level:
            fit = el[0]
            fronts[-1].extend(map_fit_ind[fit])
        pass
    size_1_front = min(len(fronts[0]), k)
    # print('Size of the first front {}'.format(size_1_front * 100. / k))
    # print('\tnum fronts {}'.format(len(fronts)))
    return fronts


def selPO(individuals, k, nd='counting'):
    """Apply PO selection operator on the *individuals*. Usually, the
    ...
    # print ("XXX")
    if nd == 'counting':
        pareto_fronts = sortPO(individuals, k)
    elif nd == 'prob':
        pareto_fronts = sortPOprob(individuals, k)
    else:
        raise Exception('selPO: The choice of non-dominated sorting '
                        'method "{0}" is invalid.'.format(nd))

    tmp = list(chain(*pareto_fronts[:]))
    if len(tmp) == k:
        chosen = tmp
    else:
        # assign crowded distance
...
def calculate_rank(vector):
  ...
  for num in sorted(vector):
    if num not in a:
      a[num]=rank
      rank=rank+1
  return[float(a[i]) for i in vector]

def total_ranks(values):
    avg = []
...
def get_distance(values):
    sum = 0
    for i in values:
        sum += i*i
    return math.sqrt(sum)

def calculate_niche_radius(front):
    sum = 0
    length = len(front)
    if length == 1:
        return 0.0
    for i in range(length-1):
        for j  in  range(i+1,length):
            sum+= get_distance(tuple(x-y for x, y in zip(front[i].fitness.values, front[j].fitness.values)))
...
def assigning_priority_rank(front, pr, radius):
    flag = False
    for i in range(len(front)):
        if type(front[i].fitness.rank) !=int:
            front[i].fitness.rank = pr
            flag = True
            for j in range(i+1, len(front)):
                if get_distance(tuple(x-y for x, y in zip(front[i].fitness.values, front[j].fitness.values)))>radius:
                    front[j].fitness.rank = pr
                    continue
            break
    if flag:
        return  assigning_priority_rank(front, pr+1, radius)
    else:
        return front
...
def selAR(individuals, k, nd='standard'):
    pareto_fronts = sortNondominated(individuals, k)

    for k in range(len(pareto_fronts)):
        niche_radius = calculate_niche_radius(pareto_fronts[k])
        rankss = []
        for i in range(len(pareto_fronts[k][0].fitness.values)):
            values = map(lambda x:x.fitness.values[i], pareto_fronts[k])

            tmp1 = list(values).copy()

            tmp = calculate_rank(tmp1)
            rankss.append(tmp)

        final_ranks_front = total_ranks(rankss)
        for i in range(len(pareto_fronts[k])):
            pareto_fronts[k][i].fitness.rank = final_ranks_front[i]
        pareto_fronts[k] = sorted(pareto_fronts[k], key=attrgetter("fitness.rank"))
        assigning_priority_rank(pareto_fronts[k], 1, niche_radius)
        pareto_fronts[k] = sorted(pareto_fronts[k], key=attrgetter("fitness.rank"))
    ...
    k = k - len(chosen)
    if k > 0:
        sorted_front = sorted(pareto_fronts[-1], key=attrgetter("fitness.rank"))
        chosen.extend(sorted_front[:k])
...
# Weighted Sum   (WS)                #
######################################
def min_values(individuals):
    min = list(individuals[0].fitness.values)
    for ind in individuals:
        for i in range(len(ind.fitness.values)):
            if min[i]>ind.fitness.values[i]:
                min[i] = ind.fitness.values[i]
    return min

def max_values(individuals):
    try:
        max = list(individuals[0].fitness.values)
    except IndexError:
        print(individuals)
    for ind in individuals:
        for i in range(len(ind.fitness.values)):
            if max[i]<ind.fitness.values[i]:
                max[i] = ind.fitness.values[i]
    return max

def normalize_objectives(individuals):
    min = min_values(individuals)
    max = max_values(individuals)

    for i in range(len(individuals)):
        tmp = []
        for j in range(len(individuals[i].fitness.values)):
            tmp.append((individuals[i].fitness.values[j] - min[j]) / (max[j] - min[j]))
        individuals[i].fitness.normalized_values = tmp.copy()
    return individuals


def selWS(individuals, k, nd='standard'):
    individuals = normalize_objectives(individuals)
    pareto_fronts = sortNondominated(individuals, k)
    ...
    for k in range(len(pareto_fronts)):
        ...
        for i in range(len(pareto_fronts[k])):
            pareto_fronts[k][i].fitness.rank = sum(list(pareto_fronts[k][i].fitness.normalized_values))
        pareto_fronts[k] = sorted(pareto_fronts[k], key=attrgetter("fitness.rank"), reverse=True)
        assigning_priority_rank(pareto_fronts[k], 1, niche_radius)
        pareto_fronts[k] = sorted(pareto_fronts[k], key=attrgetter("fitness.rank"))
    # print("XXXXXXXXXXXXX")
    # print (pareto_fronts[0][0].fitness.normalized_values)
    # print(pareto_fronts[0][1].fitness.normalized_values)
    # print(pareto_fronts[0][0].fitness.values)
    # print(pareto_fronts[0][1].fitness.values)
    chosen = list(chain(*pareto_fronts[:-1]))
    k = k - len(chosen)
    if k > 0:
        sorted_front = sorted(pareto_fronts[-1], key=attrgetter("fitness.rank"))
...
__all__ = ['selAR','selWS','selPO', 'selNSGA2', 'selNSGA3', 'selNSGA3WithMemory', 'selSPEA2', 'sortNondominated', 'sortLogNondominated',
